...
class Website(Entity):
    ...
    def to_domain(
        ...
        auth_password = None
        if record.encrypted_auth_password:
        ...
        return cls(
            ...
            auth_password=auth_password,
            auth_last_verified=record.auth_last_verified,
        )
...
class WebsiteSparse(Entity):
    ...
    def auth_password(self) -> Optional[str]:
        ...
        if not self.encrypted_auth_password:
            return None
        from intric.authentication.encryption import decrypt_password
        return decrypt_password(self.encrypted_auth_password)
    ...
    def to_domain(cls, record: "WebsitesTable") -> "WebsiteSparse":
        return cls(
            ...
            update_interval=record.update_interval,
            size=record.size,
            requires_auth=record.requires_auth or False,
            auth_username=record.auth_username,
            encrypted_auth_password=record.encrypted_auth_password,
        )
