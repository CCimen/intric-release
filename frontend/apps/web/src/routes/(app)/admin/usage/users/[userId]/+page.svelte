...
<script lang="ts">
  ...
  async function loadUserData() {
    ...
    try {
      ...
      const userStats = await intric.usage.tokens.getUsersSummary({
        startDate: dateRange.start.toString(),
        endDate: dateRange.end.add({ days: 1 }).toString()
      });
      
      // Find the specific user
      user = userStats.users.find(u => u.user_id === userId) || null;
      
      if (!user) {
        // User not found in current date range, we could redirect or show an error
  ...
  const modelTableViewModel = modelTable.createViewModel([
    modelTable.columnPrimary({
      header: "Model",
      value: (item) => item.model_nickname || item.model_name || "",
      id: "model_name"
    }),
    modelTable.column({
      header: "Organization", 
